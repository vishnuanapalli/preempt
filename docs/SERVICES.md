...
| **Docker + timescaledb-oss** | Local dev and test databases (5433 / 5434) | Local | `docker compose` | `docker:engine` `docker --version` · `docker:postgres` `server_version` · `docker:timescaledb` `pg_available_extensions` | Must use the `-oss` image so a licensed-only feature fails on the laptop rather than in production |
