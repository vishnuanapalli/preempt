...
| **Neon** | Production Postgres | Free | Connection string held by the owner; **not reachable from this machine** | presence of `PREEMPT_DATABASE_URL` in Vercel env | ~400 compute-hours/month, 0.5 GB, five-minute scale-to-zero that cannot be disabled. Only the Apache-2 subset of TimescaleDB — no compression |
