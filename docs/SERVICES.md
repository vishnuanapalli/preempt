...
| **Vercel** | Hosts the FastAPI app | Hobby (free) | `npx vercel@latest` CLI, authenticated as `vishnuanapalli-8269`; Vercel MCP for reads | `vercel whoami`, `rootDirectory`, `vercel env ls` | MCP tools are **read-only** — environment variables must be set via CLI or dashboard. Root Directory (`api`) is a dashboard setting the repo cannot express. Every push built the app, including the 9-in-10 commits that touch no code under `api/`; `ignoreCommand` in `api/vercel.json` now skips those |
