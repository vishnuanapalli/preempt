...
| **Azure Retail Prices API** | The one measured price source (D-001) | External, public | none — unauthenticated | `azure:retail-prices` HTTP 200 + an `Items` array for one region | **Rate-limits hard: HTTP 429 on a second request seconds after the first.** Tests therefore read recorded payloads and never call it; this probe is the only thing here that touches it. `$top` is ignored — pagination is via `NextPageLink` only |
